...
handle_call({send, #msg{payload = Payload}}, _From, State) ->
    io:format("Chat Server(User: ~p) - deliver msg to tcp client, Payload: ~p~n", [State#state.userid, Payload]),
...
handle_info({tcp, #msg{to_userid = ToUserID, payload = Payload} = Msg}, State) ->
    ...
    case route:lookup_server(ToUserID) of
        {error, Reason} ->
            io:format("Chat Server(User: ~p) - cannot forward to Chat Server(User: ~p): ~p~n", [State#state.userid, ToUserID, Reason]);
        {ok, TargetServerID} ->
            io:format("Chat Server(User: ~p) - forward msg to Chat Server(User: ~p), Payload: ~p~n", [State#state.userid, ToUserID, Payload]),
...
send_to_client_via_tcp(_Socket, Payload) ->
    ...
    io:format("Send to client: ~p~n", [Payload]).
